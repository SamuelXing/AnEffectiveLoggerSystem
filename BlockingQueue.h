//
// Created by samuel on 11/2/17.
//
// Purpose: Wrapping the native STL Queue to Asynchronous Blocking Queue

// TODO: more APIs

#ifndef BLOCKINGQUEUE_H
#define BLOCKINGQUEUE_H

#include <condition_variable>
#include <mutex>
#include <thread>
#include <queue>
#include <string>
#include <iostream>
#include <sstream>

namespace Async
{
    template <typename T>
    class BlockingQueue
    {
        BlockingQueue() {};
        // move constructor
        BlockingQueue(BlockingQueue<T>&& bq);
        // move assignment
        BlockingQueue<T>& operator=(BlockingQueue<T>&& bq);
        // forbid copy constructor
        BlockingQueue(const BlockingQueue<T>&) = delete;
        // forbid copy assignment
        BlockingQueue<T>& operator=(const BlockingQueue<T>&) = delete;
        T deQ();
        void enQ(const T& t);
        T& front();
        void clear();
        size_t size();
    private:
        std::queue<T>               q_;
        std::mutex                  mtx_;
        std::condition_variable     cv_;
    };

    // ----<move constructor>-----------
    template<typename T>
    BlockingQueue<T>::BlockingQueue(BlockingQueue<T> &&bq)
    {
        std::lock_guard<std::mutex> l(mtx_);
        q_ = bq.q_;
        while(bq.q_.size() > 0) // clear bq
            bq.q_.pop();
        // cannot copy or move mutex and condition variables, so use the default one.
    }

    // -----<move assignment>------------
    template<typename T>
    BlockingQueue<T>& BlockingQueue<T>::operator=(BlockingQueue<T> &&bq)
    {
        if(this == &bq) return *this;
        std::lock_guard<std::mutex> l(mtx_);
        q_ = bq.q_;
        while(bq.q_.size() > 0)
            bq.q_.pop();
        return *this;
        // cannot copy or move mutex and condition variables, so use the default one.
    }

    // -----<remove element from front of queue>-------
    template<typename T>
    T BlockingQueue<T>::deQ()
    {
        std::unique_lock<std::mutex> l(mtx_);
        /*
         * This lock type is required for use with condition variables.
         * The operating system needs to lock and unlock the mutex:
         *   - when wait is called, below, the OS suspends waiting thread and release lock
         *   - when notify is called in enQ() the OS relocks the mutex,
         *      resume the waiting thread and sets the condition variable to
         *      signaled state.
         * std::lock_guard does not have public lock and unlock functions */
        if(q_.size() > 0)
        {
            T temp = q_.front();
            q_.pop();
            return temp;
        }

        while(q_.size() == 0)
        {
            cv_.wait(l, [this](){ return q.size() > 0;});
        }
        T temp = q_.front();
        q_.pop();
        return temp;
    }

    // -------<push element onto back of queue>------
    template<typename T>
    void BlockingQueue<T>::enQ(const T &t)
    {
        {
            std::unique_lock<std::mutex> l(mtx_);
            q_.push(t);
        }
        cv_.notify_one();
    }

    // ------<peek at next item to be popped >---------
    template<typename T>
    T& BlockingQueue::front()
    {
        std::lock_guard<std::mutex> l(mtx_);
        if(q_.size() > 0)
            return q_.front();
        throw std::exception("attempt to deQue empty queue");
    }

    // -------<remove all element from queue>-----------
    template<typename T>
    void BlockingQueue::clear()
    {
        std::lock_guard<std::mutex> l(mtx_);
        while(q_.size()>0)
        {
            q_.pop();
        }
    }

    // -------<return number of element in queue>
    template<typename T>
    size_t BlockingQueue<T>::size()
    {
        std::lock_guard<std::mutex> l(mtx_);
        return q_.size();
    }
}
#endif